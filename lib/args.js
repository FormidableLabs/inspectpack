...
Validate.prototype = {
  ...
  action: function () {
    ...
    if (!this.argv.bundle && ["action", "pattern"].indexOf(action) === NOT_FOUND) {
...
module.exports = {
  parse: function () {
    return yargs
      ...
      .example(
        "$0 --action=pattern --bundle=bundle.js --suspect-patterns",
        "Show files with pattern matches"
      )
