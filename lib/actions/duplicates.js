...
/**
 * Add minified data points.
 *
 * @param {Array}     codes Indexed array of code sections
 * @param {Boolean}   minified  Add minified data?
 * @return {Function}           Mutates object values
 */
var addMinified = function (codes, minified) {
  return _.mapValues(function (group) {
    var meta = group.meta;
    ...
    _.each(function (obj, idx) {
      ...
      var minSize = minified ? uglify.minify(codeSrc, { fromString: true }).code.length : "--";
...
var textTmpl = _.template([
  ...
  "    * Pct of Bundle Size:  <%- opts.minified ? " +
...
module.exports = function (opts, callback) {
  Bundle.create(opts, function (err, bundle) {
    ...
    var duplicates = _.flow(
      ...
      addMinified(codes, opts.minified),
    ...
    duplicates.meta = {
      ...
      size: {
        ...
        min: opts.minified ? metaSum("size.min")(duplicates) : "--",
        minExtra: opts.minified ? metaSum("size.minExtra")(duplicates) : "--"
      },

      // The existing total bundle.
      bundle: {
        full: bundle.code.length,
        min: opts.minified ? uglify.minify(bundle.code, { fromString: true }).code.length : "--"
      }
    };
