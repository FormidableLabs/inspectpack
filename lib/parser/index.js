  ...
  webpackSections.forEach(element => {
    ...
    if (concatMatch) {
      ...
      if (curFileName !== baseFileName) {
        const err = JSON.stringify({ curFileName, baseFileName });
        throw new Error(`Final concat part did not module name: ${err}`);
      } else if (numAddlModules + 1 !== Object.keys(concatParts).length) {
        const err = JSON.stringify({
          numAddlModules,
          extractedNames: Object.keys(concatParts)
        });
        throw new Error(`Could not extract all concat parts: ${err}`);
      }
